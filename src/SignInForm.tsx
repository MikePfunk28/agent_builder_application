      ...
      <button
        ...
        onClick={() => {
          void signIn( "cognito" ).catch( ( error ) => {
            ...
            if ( error.message?.includes( "callback" ) || error.message?.includes( "redirect" ) ) {
              toast.error( "Cognito OAuth callback URL mismatch. Check app client settings.", {
                description: "Expected callback URL not configured in AWS Cognito User Pool.",
              } );
            } else if ( error.message?.includes( "environment" ) || error.message?.includes( "issuer" ) ) {
              toast.error( "Cognito OAuth not configured", {
                description: "Missing COGNITO_ISSUER_URL, COGNITO_CLIENT_ID, or COGNITO_CLIENT_SECRET.",
              } );
            } else {
