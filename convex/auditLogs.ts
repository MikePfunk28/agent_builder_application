 ...
 * Internal-only: not callable from clients to prevent log spoofing
 */
...
export const getAllLogs = query( {
  ...
  handler: async ( ctx, args ) => {
    ...
    // Enforce admin-only access before returning any audit logs.
    const user = await ctx.db.get( userId );
    if ( !user || ( user as any ).role !== UserRole.ADMIN ) {
      // Non-admins are not allowed to view global audit logs.
      return [];
    }
