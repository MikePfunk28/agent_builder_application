...
export function generateRequirementsTxt(tools: any[], deploymentType: string): string {
  ...
  if (isAWSDeployment(deploymentType)) {
    ...
    // Pin to 0.1.0 for agent-core-code-interpreter compatibility
    packages.set('bedrock-agentcore', '==0.1.0');
    packages.set('bedrock-agentcore-starter-toolkit', '==0.1.24');
  }
...
export function generateDeploymentReadme(agent: Agent, deploymentTarget: string, options: any): string {
  const sanitizedName = sanitizeAgentName(agent.name);
  const pythonFileName = "agent.py";
  const hasAlias = pythonFileName !== "agent.py";
  const agentFileLine = hasAlias
    ? `- \`${pythonFileName}\` - Main agent implementation (also available as \`agent.py\`)`
...
# Run locally
docker run -p 8000:8000     -e AWS_REGION=us-east-1     -e AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID     -e AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY     ${sanitizedName}:latest
\`\`\`
...
To change the model, update the \`MODEL_ID\` environment variable or modify \`${pythonFileName}\`.
